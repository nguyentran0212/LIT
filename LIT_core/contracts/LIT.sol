...
contract LIT {
    ...
    function getRespondedTestSites(bytes20 _expID) public view returns (address[] memory) {
        address[] memory testsites = experiments[_expID].respondedTestSites;
        return testsites;
    }
    
    function getExperimentResults(bytes20 _expID, address _addrTestSite) public view returns (string memory)  {
